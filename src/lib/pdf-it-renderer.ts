...
export async function generateIncomeTaxPDF(
  ...
  drawQRow('32', 'IF A PUMP INSTALLED, WHO HAS TO BEAR THE COST AND MAINTENANCE AND OPERATION,–OWNER OR TENANT.', fields.pumpMaintenance);
  drawQRow('33', 'WHO IS TO BEAR THE COST OF ELECTRICITY CHARGES FOR LIGHTING OF COMMON SPACE LIKE ENTRANCE HALL, STAIRS, PASSAGES, COMPOUND, ETC.–OWNER OR TENANT.', fields.commonElectricity);
  drawQRow('34', 'WHAT IS THE AMOUNT OF PROPERTY TAX? WHO IS TO BEAR IT? GIVE DETAILS WITH DOCUMENTARY PROOF.', fields.propertyTax);
  drawQRow('35', 'IS THE BUILDING INSURED? IF SO, GIVE THE POLICY NO., AMOUNT FOR WHICH IT IS INSURED AND ANNUAL PREMIUM.', fields.buildingInsured);
  drawQRow('36', 'IF ANY DISPUTE BETWEEN LAND LORD AND TENANT REGARDING RENT PENDING IN COURT OF LAW?', fields.landlordTenantDispute);
  drawQRow('37', 'HAS ANY STANDARD RENT BEEN FIXED FOR THE PREMISES UNDER ANY LAW RELATING TO CONTROL OF RENT', fields.standardRent);

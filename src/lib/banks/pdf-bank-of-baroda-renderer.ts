...
export class PDFBankOfBarodaRenderer extends PDFBankRenderer {
  ...
  private drawBobSection2() {
    ...
    const styleOpts = (ri: number, ci: number) => {
      ...
      const isSubTableRow = row[1] && ['East', 'West', 'North', 'South'].includes(row[1]);
  ...
  private drawBobSection3() {
    ...
    let allRows: string[][] = [
      ...
      ['3.', 'Possibility of frequent flooding / sub-merging', this.fv('bobFloodingPossibility') || 'NA'],
      ['4.', 'Feasibility to the Civic amenities like school, hospital, bus stop, market etc.', this.fv('bobCivicAmenities') || 'NA'],
      ['5.', 'Level of land with topographical Conditions', this.fv('bobLevelOfLand') || 'NA'],
      ['6.', 'Shape of land', this.fv('bobShapeOfLand') || 'NA'],
      ['7.', 'Type of use to which it can be put', this.fv('bobTypeOfUse') || 'NA'],
      ['8.', 'Any usage restriction', this.fv('bobUsageRestriction') || 'NA'],
      ['9.', 'Is plot in town planning approved layout?', this.fv('bobTownPlanningApproved') || 'NA'],
      ['10.', 'Corner plot or intermittent plot?', this.fv('bobCornerOrIntermittent') || 'NA'],
      ['11.', 'Road facilities', this.fv('bobRoadFacilities') || 'NA'],
      ['12.', 'Type of road available at present', this.fv('bobTypeOfRoad') || 'NA'],
      ['13.', 'Width of road – is it below 20 ft. or more than 20 ft.', this.fv('bobWidthOfRoad') || 'NA'],
      ['14.', 'Is it a land – locked land?', this.fv('bobLandLocked') || 'NA'],
      ['15.', 'Water potentiality', this.fv('bobWaterPotentiality') || 'NA'],
      ['16.', 'Underground sewerage system', this.fv('bobSewerage') || 'NA'],
      ['17.', 'Is power supply available at the site?', this.fv('bobPowerSupply') || 'NA'],
      ['18.', 'Advantage of the site', this.fv('bobAdvantageOfSite') || 'NA'],
      ['19.', 'Special remarks, if any, like threat of acquisition of land for public service purposes, road widening or applicability of CRZ provisions etc. (Distance from sea-coast / tidal level must be incorporated)', this.fv('bobSpecialRemarks') || 'NA'],
    ];

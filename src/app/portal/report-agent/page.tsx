...
export default async function ReportAgentDashboard() {
  ...
  if (!session?.user?.id || (session.user as any).role !== 'REPORT_EMPLOYEE') return null;
  ...
  const assignedProjects = reportProjects.slice(0, 5);
  const stats = {
    total: reportProjects.length,
    active: reportProjects.filter((p) => ['REPORT_DRAFTING', 'MANAGER_REVIEW'].includes(p.status)).length,
    completed: reportProjects.filter((p) => p.status === 'COMPLETED').length,
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="card p-5">
          <p className="text-xs font-medium text-[#6c757d] uppercase tracking-wider mb-1">Active Drafts</p>
      ...
      {assignedProjects.length === 0 && (
        <div className="card p-12 text-center">
          ...
          <p className="text-sm text-[#6c757d]">You have no active reports to draft.</p>
        </div>
      )}
    </div>
  );

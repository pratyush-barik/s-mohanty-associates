...
export default function EndProjectButton({ projectId, projectCode, variant, reportSentToClient }: EndProjectButtonProps) {
  ...
  const wordCount = reason.trim() ? reason.trim().split(/\s+/).filter(Boolean).length : 0;

  // ─── Shared modal JSX (inlined to prevent re-mount on state change) ───
          ...
          {!reportSentToClient && (
            <div>
              ...
              <p className={`text-[10px] mt-1 text-right ${
                wordCount >= 28 ? 'text-red-500 font-bold' : 'text-[#adb5bd]'
              }`}>
                {wordCount}/30 words
              </p>
            </div>

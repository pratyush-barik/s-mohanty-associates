...
export default async function ManagerDashboard() {
  ...
  if (!session?.user?.id || (session.user as any).role !== 'MANAGER') return null;
  ...
  const [pendingRequests, managerProjects, pendingTransfers] = await Promise.all([
      ...
      {pendingRequests.length > 0 && (
        <div className="card p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-[#0f2038]" style={{ fontFamily: 'var(--font-heading)' }}>Pending Service Requests</h2>
            <Link href="/portal/requests" className="text-sm text-[#b8860b] hover:underline font-medium">View All →</Link>
          </div>

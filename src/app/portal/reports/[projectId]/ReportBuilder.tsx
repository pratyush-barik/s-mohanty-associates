...
export default function ReportBuilder({ projectId, initialFields, status, userRole = 'REPORT_EMPLOYEE', prefill }: ReportBuilderProps) {
  ...
  function generatePDFPages(): string[] {
    ...
    // Label column: neutral light gray — NOT blue (blue is only for the selected option in col 2)
    const lbl = 'border:1px solid #AAAAAA;padding:4px 6px;font-size:9px;font-weight:bold;background:#F0F0F0;text-align:left;color:#1F4E78;vertical-align:middle;';
    // Option column (list of choices)
    const optS = 'border:1px solid #AAAAAA;padding:0;font-size:8.5px;vertical-align:top;background:#FFFFFF;';
    // Value column
    const valS = 'border:1px solid #AAAAAA;padding:4px 6px;font-size:9px;vertical-align:middle;color:#111;';
    // Table section header (Dark Blue #1F4E78, White text — per spec)
    ...
    // ── Alternating row colors (applies to value/simple rows only, not label or option columns) ──
    let rowIdx = 0;
    const resetRowIdx = () => { rowIdx = 0; };
    const rowBg = () => { const bg = rowIdx % 2 === 0 ? '#FFFFFF' : '#EEF4FB'; rowIdx++; return bg; };

    // ── Row helpers ───────────────────────────────────────────────────────────
    // simpleRow: label always neutral gray, value cell alternates
    const simpleRow = (label: string, val: string, bgOverride?: string) => {
      const bg = bgOverride !== undefined ? bgOverride : rowBg();
      return `<tr><td style="${lbl}" width="30%">${label}</td><td style="${valS};background:${bg};" colspan="2">${val || 'N/A'}</td></tr>`;
    };
    // optionRow: label neutral gray, options column white with ONLY selected option highlighted blue, value column white
    const optionRow = (label: string, opts: string[], val: string) => {
      rowIdx++; // count the row for alternating but don't apply to option rows
      return `<tr><td style="${lbl}" width="30%">${label}</td><td style="${optS}" width="32%">${opts.map(o => `<div style="border-bottom:1px solid #CCCCCC;padding:3px 5px;${o === val ? 'font-weight:bold;background:#C5DDF0;color:#1F4E78;' : 'background:#FFFFFF;color:#333;'}">${o}</div>`).join('')}</td><td style="${valS};background:#FFFFFF;">${val ? `<b style="color:#1F4E78;">${val}</b>` : 'N/A'}</td></tr>`;
    };
    ...
    const page2 = `<div style="font-family:Calibri,Arial,sans-serif;color:#111;line-height:1.15;">
      ...
      <table style="${ts}">
        ...
        <tr><td style="${lbl}" width="30%">Proximity to Civic Amenities</td><td style="${optS}" width="32%">
          <div style="border-bottom:1px solid #CCCCCC;padding:3px 5px;background:#FFFFFF;color:#333;">Nearest Railway Station</div>
          <div style="border-bottom:1px solid #CCCCCC;padding:3px 5px;background:#FFFFFF;color:#333;">Nearest Bus Stop</div>
          <div style="padding:3px 5px;background:#FFFFFF;color:#333;">Nearest Hospital</div>
        </td><td style="${valS};background:#FFFFFF;">
        ...
        <tr><td style="${lbl}">Landmark Details</td><td style="${optS}">
          <div style="border-bottom:1px solid #CCCCCC;padding:3px 5px;background:#FFFFFF;color:#333;">Nearest Railway Station</div>
          <div style="border-bottom:1px solid #CCCCCC;padding:3px 5px;background:#FFFFFF;color:#333;">Nearest Bus Stop</div>
          <div style="border-bottom:1px solid #CCCCCC;padding:3px 5px;background:#FFFFFF;color:#333;">Nearest Hospital</div>
          <div style="padding:3px 5px;background:#FFFFFF;color:#333;">Nearest Landmark</div>
        </td><td style="${valS};background:#FFFFFF;">

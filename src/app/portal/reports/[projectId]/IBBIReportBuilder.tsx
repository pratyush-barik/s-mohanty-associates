...
// ─── Types ─────────────────────────────────────────────────────────
const MODEL_CODE_TEXT = `MODEL CODE OF CONDUCT FOR VALUERS
...
22. A Valuer shall appear, co-operate and be available for inspections and investigations carried out by the authority, any person authorized by the authority, the registered values’ organization with which he/it is registered or any other statutory regulatory body.
...
const DEFAULT_FIELDS: IBBIFields = {
  ...
  customDeclarations: [],
  modelCodeOfConduct: MODEL_CODE_TEXT,
...
Our valuation is based on information obtained from the client and on data gathered out of our reasonable local enquiry. We have relied on this being correct & complete and there is no undisclosed matters which would affect the cause, from client's side.`;
      ...
      // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      if (fields.modelCodeOfConduct && fields.modelCodeOfConduct.trim()) {
        r.newPage();
        r.drawSectionHeader('MODEL CODE OF CONDUCT FOR VALUERS');
        tocPageMap['MODEL CODE OF CONDUCT FOR VALUERS'] = r.getPageCount();
        r.advanceCursor(6);
        const lines = fields.modelCodeOfConduct.split('\n');
        for (const line of lines) {
          if (!line.trim()) {
            r.advanceCursor(4);
            continue;
          }
          if (line.match(/^[a-zA-Z\s&]+$/) && !line.includes('Adopted in line with') && !line.startsWith('All Valuers')) {
            // Probably a subheading like "Integrity and Fairness"
            r.advanceCursor(4);
                ...
                {!isReadOnly && !fields.coverPageImage && (
                  <div className="flex items-center gap-3 mb-3">
                    <label className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-[#b8860b] text-[#b8860b] text-sm font-medium cursor-pointer hover:bg-[#b8860b]/5 transition-colors">
                      {uploading ? 'Uploading...' : '📷 Upload Cover Photograph'}
          ...
          <Section title="Declaration and Undertaking" id="section-declaration">
            <div className="space-y-4">
              ...
              <div className="mt-8 border-t border-gray-200 pt-6">
                <Field label="Model Code of Conduct for Valuers">
                  <textarea rows={15} value={fields.modelCodeOfConduct || ''} onChange={e => handleChange('modelCodeOfConduct', e.target.value)} className={inputCls} disabled={isReadOnly} />
                </Field>
              </div>

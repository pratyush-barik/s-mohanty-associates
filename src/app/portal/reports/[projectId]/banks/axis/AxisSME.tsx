...
import {
  ...
  AxisSmeReportFields,
  AxisSmeFloorItem,
} from '@/lib/banks/pdf-axis-sme-renderer';

export const AXIS_SME_CONFIG: BankConfig = {
  bankId: 'AXIS BANK',
  subTemplateId: 'SME',
  displayName: 'Axis Bank — SME',
  defaultValues: {
    purpose: 'Valuation of Property for Credit Facilities',
  },
};

export interface AxisSmeProps {
  projectId: string;
  projectCode: string;
  initialFields: any;
  status: string;
...
export default function AxisSME({
  projectId,
  projectCode,
  initialFields,
  status,
  ...
  const initialData: AxisSmeReportFields = useMemo(() => {
  ...
  const [fields, setFields] = useState<AxisSmeReportFields>(() => {
  ...
  const handlePositiveDecimalChange = useCallback((key: keyof AxisSmeReportFields, val: string) => {
    const cleaned = sanitizePositiveFloat(val);
    handleChange(key, cleaned);
  }, [handleChange]);

  const handleMatrixCellChange = useCallback((key: keyof AxisSmeReportFields, val: string) => {
    if (val.trim() === '-') {
      handleChange(key, '-');
      return;
    }
  ...
  const handleToggleMulti = useCallback((key: keyof AxisSmeReportFields, item: string) => {
    setFields(prev => {
  ...
  // Floor manipulation
  const handleFloorChange = useCallback((index: number, field: keyof AxisSmeFloorItem, val: string) => {
    setFields(prev => {
  ...
  const handlePreviewPDF = async () => {
    ...
    if (previewWindow) {
      previewWindow.document.write(`
        <html>
          <head><title>Generating Axis SME PDF Preview...</title></head>
          <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; display: flex; align-items: center; justify-content: center; height: 100vh; margin: 0; background: #f8f9fa; color: #495057;">
            <div style="text-align: center;">
              ...
              <p style="font-size: 16px; font-weight: 600; margin: 0;">Generating Axis SME PDF Preview...</p>
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `Axis_SME_Valuation_${projectCode || 'Report'}.pdf`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
  ...
  const navSections: NavItem[] = [
    { id: 'sec-1', title: 'Header & Initiation' },
    { id: 'sec-2', title: 'Property Location & Details' },
    { id: 'sec-3', title: 'Type of Property' },
    { id: 'sec-4', title: 'Boundaries' },
    { id: 'sec-5', title: 'Locality & Structure' },
    { id: 'sec-6', title: 'Tenancy & Leasehold' },
    { id: 'sec-7', title: 'Statutory Approvals' },
    { id: 'sec-8', title: 'Construction & Building Details' },
    { id: 'sec-9', title: 'Land Rate Adopted' },
    { id: 'sec-10', title: 'Building Valuation Breakdown' },
    { id: 'sec-11', title: 'Value of Property Summary' },
    { id: 'sec-12', title: 'Basis of Valuation & Remarks' },
    { id: 'sec-13', title: 'Valuer Declaration & Undertaking' },
    { id: 'sec-14', title: 'Annexure "A"' },
    { id: 'sec-15', title: 'Valuation Report Checklist' },
    { id: 'sec-16', title: 'Documents' },
    { id: 'sec-17', title: 'Maps' },
    { id: 'sec-18', title: 'Property Photographs' },
  ];
